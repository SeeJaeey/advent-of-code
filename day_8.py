...
for i in range(len(outputs)):
    ...
    signal = signals[i]
    mapping = [[], [], [], [], [], [], [], [], [], []]
    # Search for digits 1, 4, 7, 8
    for digit in signal:
        # DÜRFEN HIER WAHRSCHEINLICH DOCH NICHT SORTIEREN WEIL DIE REIHENFOLGE DIE KODIERUNG BESTIMMT

        if (len(digit) == 2):
            mapping[1] = ([digit[0], digit[1]])
        if (len(digit) == 3):
            mapping[7] = ([digit[0], digit[1], digit[2]])
        if (len(digit) == 4):
            mapping[4] = ([digit[0], digit[1], digit[2], digit[3]])
        if (len(digit) == 7):
            mapping[8] = ([digit[0], digit[1], digit[2], digit[3], digit[4], digit[5], digit[6]])
    
    # Deduct 9 (subtract 4 and 7 from digits that contain 6 chars)
    for digit in signal:
        if (len(digit) == 6):
            leftover = [d for d in digit if d not in mapping[4] and d not in mapping[7]]
            if (len(leftover) == 1):
                mapping[9] = [d for d in digit]
                break

    # Deduct 6
    for digit in signal:
        if (len(digit) == 6):
            leftover = [d for d in digit if d not in mapping[1]]
            if (len(leftover) == 5):
                mapping[6] = [d for d in digit]
                break
    ...
    for digit in signal:
        if (len(digit) == 6):
            dd = [d for d in digit]
            if (dd != mapping[6] and dd != mapping[9]):
                mapping[0] = dd
                break
    ...
    for digit in signal:
        if (len(digit) == 5):
            leftover = [d for d in digit if d not in mapping[9]]
            if (len(leftover) == 1):
                mapping[2] = [d for d in digit]
                break

    # Deduct 5
    for digit in signal:
        if (len(digit) == 5):
            leftover = [d for d in digit if d not in mapping[2]]
            if (len(leftover) == 2):
                mapping[5] = [d for d in digit]
                break
    ...
    for i in range(len(output)):
        out = [o for o in output[i]]
        #print(out)
        for j in range(len(mapping)):
            if (sorted(mapping[j]) == sorted(out)):
                output_digits.append(j)
                break

    assert(len(output_digits) == 4)

    result += output_digits[3] + 10 * output_digits[2] + 100 * output_digits[1] + 1000 * output_digits[0]
